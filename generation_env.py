...
def non_stat_Lopes_article(transitions, index_state_to_change):
    ...
    for action in range(5):
        copy_transitions[changed_state][derangement_list[action]] = transitions[changed_state][action]
...
def proportion_of_valid_worlds(iterations=2000, alphas=[1, 0.1], malus=-0.1, bonus=1):
    ...
    print('')
    print('Percentage of valid worlds out of ' + str(iterations) + ' sampled worlds, with a malus of ' +
          str(malus)+': '+str(round(100*valid_count/iterations, 1))+'%')
...
def generate_valid_stationary_environments(number_of_worlds=1, alphas=[1, 0.1], malus=-0.1, bonus=1):
    for index_world in range(1, number_of_worlds + 1):
        ...
        while not validity and counter < 1000:
            ...
            counter += 1
        if counter == 1000:
            raise RuntimeError('No world found with the optimal policy')
        np.save('Environments/Transitions_Lopes_' + str(malus) +
                '_'+str(index_world) + '.npy', transitions)
...
def generate_non_stationarity_article(world_number=1, number_of_worlds=1, malus=-0.1):
    for index_world in range(1, world_number+1):
        transitions = np.load('Environments/Transitions_Lopes_'+str(malus) +
