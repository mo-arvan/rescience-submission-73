...
class World_Representation:
    ...
    def __init__(self, environment, table=np.zeros((0,0)),actions=np.zeros((0,0)),title='Best policy'):
        ...
        pygame.init()
        pygame.font.init()
        self.size = (5*self.size_coeff+self.margin, 5*self.size_coeff+self.margin)
        ...
        if table.size >0:
            for row in range(len(table)):
                for col in range(len(table[0])): 
                    ...
                    pygame.draw.rect(self.screen,self.color,[(self.size_coeff)*col+self.margin,(self.size_coeff)*row+self.margin,
                                                                 self.size_coeff-self.margin,self.size_coeff-self.margin])
                    x,y=self.size_coeff*col+55,self.size_coeff*row+55
                    best_action=actions[row,col]
                    if best_action==STAY:
                        pygame.draw.circle(self.screen, self.BLACK, (x,y), 16)
                    if best_action!= STAY : 
                        angles_arrow=[0,180,90,270]
                        arrow_alignment={UP:(0,15),DOWN:(0,-15),LEFT:(10,0),RIGHT:(-10,0)}
        ...
        self.screen.blit(starting_state,(70,60))
        goal_state=self.font.render("G",1,self.BLACK)
        self.screen.blit(goal_state,(470,260))
    ...
    def DrawArrow(self,x,y,color,angle=0):
        def rotate(pos, angle):	
            ...
            return ret		
        p0 = rotate((0+x,-16+y), angle+90)
        p1 = rotate((0+x,16+y), angle+90)
        p2 = rotate((32+x,0+y), angle+90)
