...
def loading_environments():
    ...
    for number_non_stationarity in range(1, 21):
        transitions_non_stat_article = np.load(
            'Environments/Transitions_non_stat_article-0.1_1_'+str(number_non_stationarity)+'.npy', allow_pickle=True)
        transitions_strong_non_stat = np.load(
            'Environments/Transitions_strong_non_stat_-0.1_1_'+str(number_non_stationarity)+'.npy', allow_pickle=True)
        environments_parameters["Non_stat_article_-0.1_{0}".format(number_non_stationarity)] = {
            'transitions': transitions_lopes, 'rewards': reward_0_1, 'transitions_after_change': transitions_non_stat_article}
        environments_parameters["Non_stat_strong_-0.1_{0}".format(number_non_stationarity)] = {
            'transitions': transitions_lopes, 'rewards': reward_0_1, 'transitions_after_change': transitions_strong_non_stat}

    for number_world in range(1, 11):
        transitions_lopes = np.load('Environments/Transitions_Lopes_-1_' +
                                    str(number_world)+'.npy', allow_pickle=True)
        environments_parameters['Stationary_Lopes_-1_' +
                                str(number_world)] = {'transitions': transitions_lopes, 'rewards': reward_1}
        for number_non_stationarity in range(1, 11):
            transitions_non_stat_article = np.load('Environments/Transitions_non_stat_article-1_'+str(
                number_world)+'_'+str(number_non_stationarity)+'.npy', allow_pickle=True)
            transitions_strong_non_stat = np.load('Environments/Transitions_strong_non_stat_-1_'+str(
                number_world)+'_'+str(number_non_stationarity)+'.npy', allow_pickle=True)
            environments_parameters["Non_stat_article_-1_{0}".format(number_world)+'_{0}'.format(number_non_stationarity)] = {
                'transitions': transitions_lopes, 'rewards': reward_1, 'transitions_after_change': transitions_non_stat_article}
            environments_parameters["Non_stat_strong_-1_{0}".format(number_world)+'_{0}'.format(number_non_stationarity)] = {
                'transitions': transitions_lopes, 'rewards': reward_1, 'transitions_after_change': transitions_strong_non_stat}
    ...
    for trial in range(trials):
        ...
        while not game_over:
            ...
            if agent.step_counter % step_between_VI == 0:
                optimal_policy_value_error.append(policy_evaluation(environment, get_optimal_policy(
                    agent, agent.gamma, accuracy_VI), agent.gamma, accuracy_VI)[environment.first_location]-val_iteration[environment.first_location])
                real_policy_value_error.append(policy_evaluation(environment, get_agent_current_policy(
                    agent), agent.gamma, accuracy_VI)[environment.first_location]-val_iteration[environment.first_location])
...
def get_mean_and_SEM(dictionary, names_environments, agents_tested, number_of_iterations):
    ...
    mean = {name_agent: np.average([dictionary[name_environment, name_agent, i] for i in range(
        number_of_iterations) for name_environment in names_environments], axis=0) for name_agent in agents_tested}
    SEM = {name_agent: np.std([dictionary[name_environment, name_agent, i] for name_environment in names_environments for i in range(
        number_of_iterations)], axis=0)/np.sqrt(number_of_iterations*len(names_environments)) for name_agent in agents_tested}
...
def extracting_results(rewards, opt_pol_error, real_pol_error, names_environments, agents_tested, number_of_iterations):
    ...
    optimal_statistics = get_mean_and_SEM(
        opt_pol_error, names_environments, agents_tested, number_of_iterations)
    real_statistics = get_mean_and_SEM(real_pol_error, names_environments,
                                       agents_tested, number_of_iterations)
    rewards_statistics = get_mean_and_SEM(
        rewards, names_environments, agents_tested, number_of_iterations)
...
def compute_optimal_policies():
    env_params = loading_environments()
    for name_env in env_params.keys():
        if 'transitions_after_change' in env_params[name_env].keys():
            env_params[name_env]['transitions'] = env_params[name_env]['transitions_after_change']
        environment = Lopes_environment(**env_params[name_env])
...
# Sketch of an interactive tool to launch the simulations
"""def ask_for_input():
    print("\n"
          "Please indicate the number of the experiment to launch.\n"
          "\n"
          "0: all experiments. 1: stationary. 2: wrong prior.\n"
          "3: small task change. 4: strong task change. 5: non informative prior.\n")

    experiment_name = input("Enter the experiment number here: ")
    if experiment_name not in ['0', '1', '2', '3', '4', '5']:
        raise Exception("Please enter a number between 0 and 5")
    ...
    print("\n"
          "Please choose the set of optimized parameters to use.\n"
          "\n"
          "0: all sets.\n"
          "1 : optimal policy error, punishment of -0.1.\n"
          "3: real policy error, punishment of -0.1.\n"
          "4: optimal policy error, punishment of -1.\n")

    agent_parameters = input("Enter the number of the set of parameters here: ")
    if agent_parameters not in ['0', '1', '2', '3']:
        raise Exception("Please enter a number between 0 and 3")
    agent_parameters = int(agent_parameters)

    return experiment_name, agent_parameters


def run_an_experiment(experiment_name, param_number):
    sets_of_parameters = {
        1:
    ...
    if experiment_name == 1:
        ...
        if param_number == 1:
            agent_parameters = sets_of_parameters[param_number]
            environments = ['Lopes']
            nb_iters = 20
            starting_seed = 100
